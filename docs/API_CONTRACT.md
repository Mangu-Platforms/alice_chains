...
| G-22 | ~~**Dead modules on the API surface:** `api/lib/http.ts`, `api/lib/cookies.ts`, `api/kimi/platform.ts`, `getSessionCookieOptions`, `getIO`/`getOnlineUsers`, `isProduction`/`getPort`/`getOwnerUnionId` — all defined, none imported anywhere~~ **Closed.** `api/lib/http.ts` went with H-2, `getIO` gained call sites with F-1's `api/lib/realtime.ts`, `getOwnerUnionId` with S-18, `api/lib/cookies.ts` was rewritten as the single `Set-Cookie` emitter in S-17, and `api/kimi/platform.ts` was deleted in P-TOOL-7 | repo-wide grep |
